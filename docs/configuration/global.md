...
Markdown 文档目录地址，Docgeni 会扫描该目录下的文件夹和 Markdown 文件，按照一定的规则生成一级导航、菜单和页面文档。
...
该配置项用于自定义导航栏和菜单的展示，一般会配置一些外部的链接和类库一级导航，如：
```ts
module.exports = {
    ...
    navs: [
...
```
> 这里需要注意的是，`docs`文件夹下自动识别的一级导航会默认插入到配置的`navs`数组的底部，如果想要控制展示的位置，可以插入一个`null`进行占位，如上示例，自动生成的一级导航会插入到导航栏的顶部。
