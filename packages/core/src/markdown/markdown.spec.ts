...
describe('markdown', () => {
    ...
    it('should call markdown config with marked instance in initializeConfig', () => {
        const config = jasmine.createSpy('markdownConfig');
        Markdown.initializeConfig({ config });
        expect(config).toHaveBeenCalled();
        expect(typeof config.calls.mostRecent().args[0].parse).toBe('function');
    });

    it('should get correct result for base and ng command', async () => {
        const data = ['```bash', 'ng add alib', '```'].join('\n');
        const result = await Markdown.toHTML(data);
        expect(result).not.toContain('%20');
        expect(result).toContain(`<span class="token function">ng</span>`);
    });

    it('should render language label and code-copy inside code-block-actions', async () => {
        const result = await Markdown.toHTML(['```typescript', 'const a = 1;', '```'].join('\n'));
    ...
    describe('full', () => {
        ...
        it('should transform full marked success', async () => {
            const output = await Markdown.toHTML(fixture.src['hello.md'], {
        ...
        it('should transform custom tag success', async () => {
            const output = await Markdown.toHTML('<label><div>content</div></label>', {});
            expect(output).toContain(`<div class="dg-paragraph"><label><div>content</div></label></div>`);
        });
    });

    describe('link', () => {
        it('should renderer link success', async () => {
            const output = await Markdown.toHTML('[book](./book)', {});
            expect(output).toContain(`<a href="./book">book</a>`);
        });

        it('should add target="_blank" for external link', async () => {
            const output = await Markdown.toHTML('[book](http://pingcode.com/book)', {});
    ...
    describe('embed', () => {
        ...
        it('should transform embed success', async () => {
            const output = await Markdown.toHTML(fixture.src['hello.md'], { absFilePath: fixture.getSrcPath('hello.md') });
            expect(strings.compatibleNormalize(output).trim()).toEqual(fixture.getOutputContent('hello.html', true));
        });

        it('should throw error when embed ref self', async () => {
            const output = await Markdown.toHTML(fixture.src['embed-self.md'], { absFilePath: fixture.getSrcPath('embed-self.md') });
            expect(output).toContain(`<div embed src="./embed-self.md">can't resolve path ./embed-self.md</div>`);
        });

        it('should throw error when embed ref-not-found.md', async () => {
            const output = await Markdown.toHTML(fixture.src['not-found.md'], { absFilePath: fixture.getSrcPath('not-found.md') });
            expect(output).toContain(`<div embed src="./ref-not-found.md">can't resolve path ./ref-not-found.md</div>`);
        });
    });

    describe('heading', () => {
        it('should get correct headings', async () => {
            const result = await Markdown.compile(`
# heading1
# heading2
`);
        ...
        it('should get correct headings when contains link', async () => {
            const result = await Markdown.compile(`# heading1 <a href="https://example.com">Link</a>`);
        ...
        it('should get one header when content + code snippet,', async () => {
            /**
